    ...
    <div className="px-6 py-6 max-w-[1400px] mx-auto space-y-6">
          ...
          <tbody className="divide-y divide-hairline">
            {runs.map((r) => (
              <tr key={r.id} className="hover:bg-accent/40 cursor-pointer">
                ...
                <td className="px-2 py-2.5 font-mono text-[11px] text-muted-foreground">{r.buildRef}</td>
                <td className="px-2 py-2.5 text-[11px] font-mono text-muted-foreground">
      ...
      <div className="rounded-lg ring-1 ring-hairline bg-card">
        <div className="px-4 py-2.5 hairline-b flex items-center gap-2">
          ...
          <span className="text-[11px] font-mono text-muted-foreground">{flaky.length} tracked</span>
        </div>
        {flaky.length === 0 ? (
            ...
            {flaky.map((f) => (
              <li key={f.caseId} className="px-4 py-3 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] text-muted-foreground font-mono">root cause: {f.rootCause}</div>
                </div>
                <div className="text-[11px] font-mono">
                  <span className="text-warn">flakiness {(f.score * 100).toFixed(0)}%</span>
                </div>
                <div className="text-[11px] font-mono text-muted-foreground">SLA: {<TimeAgo date={f.slaDueAt} />}</div>
                <div className={`text-[10px] font-mono px-1.5 py-0.5 rounded ring-1 ${f.quarantined ? "ring-warn/30 text-warn bg-warn/10" : "ring-hairline text-muted-foreground"}`}>
                  {f.quarantined ? "QUARANTINED" : "active"}
                </div>
                <Button variant="outline" size="sm" className="h-7 text-[11px]" onClick={() => toggleQ(f.caseId)}>
