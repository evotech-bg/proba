...
import { ArrowLeft, Play, Plus, Trash2, Loader2, Check, Users } from "lucide-react";
import { toast } from "sonner";
import { useProba } from "@/lib/mock/store";
import { fetchAppConfig } from "@/lib/api/proba.functions";
...
function SuiteDetail() {
  ...
  const [result, setResult] = useState<{ cases: number; casesPassed: number; casesFailed: number; variations?: { account: string; casesPassed: number; casesFailed: number }[] } | null>(null);
  const [accountNames, setAccountNames] = useState<string[]>([]);
  const [picked, setPicked] = useState<Set<string>>(new Set());

  useEffect(() => {
    const appKey = suite?.appKey;
    if (!appKey) return;
    void fetchAppConfig({ data: { appKey } }).then((c) => setAccountNames((c as { accounts: { name: string }[] }).accounts.map((a) => a.name)));
  }, [suite?.appKey]);
  ...
  const runSuite = async () => {
    ...
    try {
      ...
      const r = await replaySuiteFn({ data: { suiteId, ...(accounts.length ? { accounts } : {}) } });
      setResult(r);
      await refresh();
      const span = accounts.length ? ` across ${accounts.length} accounts` : "";
      if (r.casesFailed === 0) toast.success(`Suite passed — ${r.casesPassed}/${r.cases} runs${span}`);
      else toast.error(`Suite: ${r.casesFailed} of ${r.cases} runs failed${span}`);
    } catch (e) {
  ...
  return (
    <div className="px-6 py-5 max-w-[1000px] mx-auto space-y-4">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-2xl leading-relaxed">
            A single runnable suite. Run it to execute every case below and roll the results up into one combined
            verdict.
          </p>
        </div>
        <Button size="sm" className="h-8 gap-1.5" onClick={runSuite} disabled={running || cases.length === 0}>
          {running ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />}
          {picked.size > 0 ? `Run × ${picked.size}` : "Run suite"}
        </Button>
      ...
      {accountNames.length > 0 && (
        <div className="rounded-lg ring-1 ring-hairline bg-panel/40 p-3">
          <div className="flex items-center gap-2 text-[13px]">
            <Users className="h-3.5 w-3.5 text-muted-foreground" />
            <span className="font-medium">Run for each account</span>
            <span className="text-xs text-muted-foreground">— runs the whole suite once per selected account, binding <code className="font-mono">{`{{account.*}}`}</code> and its saved login each pass.</span>
          </div>
            ...
            {accountNames.map((n) => {
              ...
              return (
                <button
                  key={n}
                  type="button"
                  onClick={() => setPicked((s) => { const x = new Set(s); x.has(n) ? x.delete(n) : x.add(n); return x; })}
                  className={cn("inline-flex items-center gap-1 h-7 px-2.5 rounded-md text-[12px] font-mono ring-1 transition", on ? "ring-primary/40 bg-primary/10 text-primary" : "ring-hairline text-muted-foreground hover:bg-muted/50")}
                >
      ...
      {result && (
        <div className={cn("rounded-lg ring-1 p-3 text-[13px] space-y-1.5", result.casesFailed === 0 ? "ring-pass/30 bg-pass/10 text-pass" : "ring-fail/30 bg-fail/10 text-fail")}>
          <div className="flex items-center gap-2">
            {result.casesFailed === 0 ? <Check className="h-4 w-4" /> : null}
            Suite replay — {result.casesPassed} passed, {result.casesFailed} failed of {result.cases} runs.
          </div>
          {result.variations && (
            <ul className="text-[12px] font-mono space-y-0.5 pl-6">
              {result.variations.map((v) => (
                <li key={v.account}>{v.account}: {v.casesPassed} passed, {v.casesFailed} failed</li>
              ))}
            </ul>
          )}
        </div>
      ...
      <section className="rounded-lg ring-1 ring-hairline bg-card">
        <div className="flex items-center justify-between px-4 py-2.5 hairline-b">
          <h2 className="text-sm font-medium">Cases <span className="text-muted-foreground font-mono text-xs ml-1">{cases.length}</span></h2>
          ...
          <ul className="divide-y divide-hairline">
            {cases.map((t) => (
              <li key={t.id} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <span className="text-xs text-muted-foreground font-mono">{t.steps.length} steps</span>
